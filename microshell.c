...
int main(int argc, char *argv[], char *env[])
{
    ...
    while (argv[i] && argv[i + 1]) //Recorremos los argv.
    {
        ...
        else if(i != 0 && strcmp(argv[i], "|") == 0) //pipe
        {
            ...
            else//Para el padre, cerramos el fd[1], el tmp_fd y restauramos tmp_fd.
            {
                close(fd[1]);
                close(tmp_fd);
                tmp_fd = fd[0];//Establece tmp_fd para que apunte al descriptor de archivo de lectura de la tubería fd. Esto significa que ahora, la entrada estándar para el próximo comando (si lo hay) será la salida de la tubería que proviene del comando anterior.
            }
        }
    }
    close(tmp_fd);//Cerramos al final el tmp_fd.
    return (0);
}
